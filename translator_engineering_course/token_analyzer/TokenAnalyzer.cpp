...
void TokenAnalyzer::ERR(string pt)
{
	...
	string Buf = pt;
	// можно проверить если начинается с (* то присвоить номер 3000, это будут ошибки коментариев
	// можно сохранить номера незакрытых коментариев и если мы попали на такой номер то пис
	// хотя конечно лучше так не делать, но тогда как распознать незакрытый коментарий ?
	
	while ((ascii_arr[sbuff] != 0) && (ascii_arr[sbuff] != 3) && (SizeOut())) // 3 заменить на 4 ???
	{
		Buf += sbuff;
		position++;
		sbuff = file.get();
...
void TokenAnalyzer::MakeListing(string file) {
	...
	{
		if (tokens[i].code >= 2000) {
			if(find(com_err_codes.begin(), com_err_codes.end(), tokens[i].code) != com_err_codes.end()){ // finds comment errors end change error message 
				message = "Unclosed comment, error: ";
			} else {
				message = "Undefined token, error ";
			}
			f << message << tokens[i].code <<  " in line: " << tokens[i].line << " column " << tokens[i].column << ": " << tokens[i].name << endl;
			p++;
		}
	}
	f.close();
	if (p == 0) {
		...
		{
			if (tokens[i].code >= 2000) {
				cout << message << tokens[i].code <<  " in line: " << tokens[i].line << " column " << tokens[i].column << ": " << tokens[i].name << endl;
			}
		}
	}
}